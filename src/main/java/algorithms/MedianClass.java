...
/**
 * 求中位数
 * 给定两个【有序】数组arr1和arr2， 两个数组【长度都为N】，求两个数组中所有数的上中位数
 * 例如：
 * arr1 = {1,2,3,4}
 * arr2 = {3,4,5,6}
 * 一共8个数，则上中位数是第4个数，所以返回3
 *
 * arr1 = {0,1,2}
 * arr2 = {3,4,5}
 * 一共6个数则上中位数是第3个数，则返回2
...
public class MedianClass {
    ...
    public static int getUpMedian(int[] arr1, int[] arr2) {
        if (arr1 == null || arr2 == null || arr1.length != arr2.length){
            throw new RuntimeException("Your par is invalid");
        }
        return findProcess(arr1, 0, arr1.length-1, arr2, 0, arr2.length-1);
    }

    private static int findProcess(int[] arr1, int start1, int end1, int[] arr2, int start2, int end2) {
        if (start1 == end1){
